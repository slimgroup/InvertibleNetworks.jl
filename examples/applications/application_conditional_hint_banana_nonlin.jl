...
Y = Y + .2f-0*randn(Float32, nx, ny, n_in, test_size)
...
for j=1:maxiter
    ...
    X = sample_banana(batchsize)
		...
		Y = Y + .2f-0*randn(Float32, nx, ny, n_in, batchsize)
...
ax2 = subplot(2,5,6); plot(Zx_[1, 1, 1, :], Zx_[1, 1, 2, :], "g."); title(L"Latent space: $zx = f(x|y)$")
ax2.set_xlim([-3.5, 3.5]); ax2.set_ylim([-3.5, 3.5])
ax3 = subplot(2,5,2); plot(Y[1, 1, 1, :], Y[1, 1, 2, :], "g."); title(L"Noisy data $y=Ax+n$ ")
#ax3.set_xlim([-3.5, 3.5]); ax2.set_ylim([-3.5, 3.5])
ax4 = subplot(2,5,7); plot(Zy_[1, 1, 1, :], Zy[1, 1, 2, :], "."); title(L"Latent space: $zy \sim \hat{p}_{zy}$")
#ax4.set_xlim([-3.5, 3.5]); ax6.set_ylim([-3.5, 3.5])
ax5 = subplot(2,5,3); plot(Y_[1, 1, 1, :], Y_[1, 1, 2, :], "g."); title(L"Data space: $y = f(zx|zy)^{-1}$")
#ax5.set_xlim([-3.5,3.5]); ax5.set_ylim([0,50])
ax6 = subplot(2,5,8); plot(Zy[1, 1, 1, :], Zy[1, 1, 2, :], "."); title(L"Latent space: $zy \sim \hat{p}_{zy}$")
#ax6.set_xlim([-3.5, 3.5]); ax6.set_ylim([-3.5, 3.5])
ax7 = subplot(2,5,4); plot(X_[1, 1, 1, :], X_[1, 1, 2, :], "."); title(L"Model space: $x = f(zx|zy)^{-1}$")
ax8 = subplot(2,5,9); plot(Zx[1, 1, 1, :], Zx[1, 1, 2, :], ".");  title(L"Latent space: $zx \sim \hat{p}_{zy}$")
ax9 = subplot(2,5,5); plot(X_post[1, 1, 1, :], X_post[1, 1, 2, :], "g."); 
plot(X_fixed[1, 1, 1, :], X_fixed[1, 1, 2, :], "r."); title(L"Model space: $x = f(zx|zy_{fix})^{-1}$")
ax9.set_xlim([-3.5, 3.5]); ax9.set_ylim([0,50])
