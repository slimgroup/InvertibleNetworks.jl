...
###########################################Test 3D with split_scales = false #########################
...
# Network and input
G = NetworkGlow(n_in, n_hidden, L, K;ndims=3)
X = rand(Float32, nx, ny, nz, n_in, batchsize)
...
# Gradient test w.r.t. input
X = rand(Float32, nx, ny, nz, n_in, batchsize)
X0 = rand(Float32, nx, ny, nz, n_in, batchsize)
dX = X - X0
...
# Gradient test w.r.t. parameters
X = rand(Float32, nx, ny, nz, n_in, batchsize)
G0 = NetworkGlow(n_in, n_hidden, L, K; ndims=3)
Gini = deepcopy(G0)
