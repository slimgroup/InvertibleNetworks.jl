...
for split_scales in [true,false]
    ...
    # Network and input
    G = NetworkConditionalGlow(n_in, n_cond, n_hidden, L, K;split_scales=split_scales)
    ...
    X = rand(Float32, nx, ny, n_in, batchsize)
    G = NetworkConditionalGlow(n_in, n_cond, n_hidden, L, K;split_scales=split_scales)
    G0 = NetworkConditionalGlow(n_in, n_cond, n_hidden, L, K;split_scales=split_scales)
    Gini = deepcopy(G0)
