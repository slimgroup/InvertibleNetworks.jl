...
G.CL[1,1].forward(rand(Float32, nx, ny, 8, batchsize),rand(Float32, nx, ny, 8, batchsize))

Y, Cond_z = G.forward(X,Cond)
X_,Cond_ = G.inverse(Y,Cond_z)

@test isapprox(norm(X - X_)/norm(X), 0f0; atol=1f-5)

###################################################################################################
# Test gradients are set and cleared
G.backward(Y, Y, Cond_z)
...
X = rand(Float32, nx, ny, n_in, batchsize)
G = NetworkConditionalGlow(n_in, n_cond, n_hidden, L, K;split_scales=true)
G0 = NetworkConditionalGlow(n_in, n_cond, n_hidden, L, K;split_scales=true)
Gini = deepcopy(G0)
...
########################################### Test with split_scales = true and REV #########################
# Invertibility

# Network and input
G = NetworkConditionalGlow(n_in, n_cond, n_hidden, L, K; logdet=false, split_scales=true)
G = reverse(G)
X = rand(Float32, nx, ny, n_in, batchsize)
Cond = rand(Float32, nx, ny, n_cond, batchsize)


Y_, Cond_ = G.inverse(X,Cond)
X_, Cond_z = G.forward(Y_,Cond_)

@test isapprox(norm(X - X_)/norm(X), 0f0; atol=1f-5)

###################################################################################################
# Test gradients are set and cleared
G.backward(X_, X_, Cond_z)
...
@test isequal(gsum, L*K*10) #maybe we need to set act? Definitely. not in split_scales
...
X = rand(Float32, nx, ny, n_in, batchsize)
G = NetworkConditionalGlow(n_in, n_cond, n_hidden, L, K; logdet=false, split_scales=true)
G0 = NetworkConditionalGlow(n_in, n_cond, n_hidden, L, K; logdet=false, split_scales=true)
Gini = deepcopy(G0)
...
ZX, ZCond = G.forward(X,Cond)
X_ = G.inverse(ZX,ZCond)[1]

@test isapprox(norm(X - X_)/norm(X), 0f0; atol=1f-5)

###################################################################################################
# Test gradients are set and cleared
G.backward(ZX, ZX, ZCond)
