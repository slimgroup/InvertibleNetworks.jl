...
# Observed data
Y, logdet = CS.forward(X, A, D)

function loss(CS, X, A, D)
    Y, logdet = CS.forward(X, A, D)
    f = -log_likelihood(Y) - logdet
    ΔY = -∇log_likelihood(Y)
    ΔX = CS.backward(ΔY, Y, A, D)[1]
    return f, ΔX, CS.RB.W1.grad
end

# Gradient test for input
f0, gX = loss(CS, X0, A, D)[1:2]
h = 0.1f0
maxiter = 6
