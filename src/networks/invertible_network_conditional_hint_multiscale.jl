...
function NetworkMultiScaleConditionalHINT(nx, ny, n_in, batchsize, n_hidden, L, K; k1=4, k2=3, p1=0, p2=1)
    ...
    XY_dims = Array{Tuple}(undef, L-1)   # save dimensions for inverse/backward pass
    ...
    return NetworkMultiScaleConditionalHINT(AN_X, AN_Y, CL, XY_dims, 
        (X, Y) -> cond_mshint_forward(X, Y, AN_X, AN_Y, CL, XY_dims, L, K),
        (Zx, Zy) -> cond_mshint_inverse(Zx, Zy, AN_X, AN_Y, CL, XY_dims, L, K),
        (ΔZx, ΔZy, Zx, Zy) -> cond_mshint_backward(ΔZx, ΔZy, Zx, Zy, AN_X, AN_Y, CL, XY_dims, L, K),
        Y -> cond_mshint_forward_Y(Y, AN_Y, CL, XY_dims, L, K),
        Zy -> cond_mshint_inverse_Y(Zy, AN_Y, CL, XY_dims, L, K)
    )
end
...
function cond_mshint_forward(X, Y, AN_X, AN_Y, CL, XY_dims, L, K)
    XY_save = Array{Array}(undef, L-1, 2)
    logdet = 0f0
    for i=1:L
        X = wavelet_squeeze(X)
...
function cond_mshint_inverse(Zx, Zy, AN_X, AN_Y, CL, XY_dims, L, K)
...
function cond_mshint_backward(ΔZx, ΔZy, Zx, Zy, AN_X, AN_Y, CL, XY_dims, L, K)
...
# Forward pass and compute logdet
function cond_mshint_forward_Y(Y, AN_Y, CL, XY_dims, L, K)
    Y_save = Array{Array}(undef, L-1)
    for i=1:L
        Y = wavelet_squeeze(Y)
        for j=1:K
    ...
    Y = cat_states(Y_save, Y)
    return Y

end

# Inverse pass and compute gradients
function cond_mshint_inverse_Y(Zy, AN_Y, CL, XY_dims, L, K)
    Y_save, Zy = split_states(XY_dims, Zy)
    for i=L:-1:1
        if i < L
