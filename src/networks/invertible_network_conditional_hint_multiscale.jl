...
# Constructor
function NetworkMultiScaleConditionalHINT(nx, ny, n_in, batchsize, n_hidden, L, K; split_scales=false, k1=3, k2=3, p1=1, p2=1, s1=1, s2=1)
    ...
    return NetworkMultiScaleConditionalHINT(AN_X, AN_Y, CL, XY_dims, 
        (X, Y) -> cond_mshint_forward(X, Y, AN_X, AN_Y, CL, XY_dims, L, K, split_scales),
        (Zx, Zy) -> cond_mshint_inverse(Zx, Zy, AN_X, AN_Y, CL, XY_dims, L, K, split_scales),
        (ΔZx, ΔZy, Zx, Zy) -> cond_mshint_backward(ΔZx, ΔZy, Zx, Zy, AN_X, AN_Y, CL, XY_dims, L, K, split_scales),
        Y -> cond_mshint_forward_Y(Y, AN_Y, CL, XY_dims, L, K, split_scales),
        Zy -> cond_mshint_inverse_Y(Zy, AN_Y, CL, XY_dims, L, K, split_scales)
    )
end
...
function cond_mshint_forward(X, Y, AN_X, AN_Y, CL, XY_dims, L, K, split_scales)
    split_scales && (XY_save = Array{Array}(undef, L-1, 2))
    logdet = 0f0
    for i=1:L
        X = wavelet_squeeze(X)
        Y = wavelet_squeeze(Y)
        for j=1:K
            X_, logdet1 = AN_X[i, j].forward(X)
            Y_, logdet2 = AN_Y[i, j].forward(Y)
            X, Y, logdet3 = CL[i, j].forward(X_, Y_)
            logdet += (logdet1 + logdet2 + logdet3)
        end
        if split_scales && i < L    # don't split after last iteration
            X, Zx = tensor_split(X)
            Y, Zy = tensor_split(Y)
            XY_save[i, :] = [Zx, Zy]
            XY_dims[i] = size(Zx)
    ...
    split_scales && ((X, Y) = cat_states(XY_save, X, Y))
    return X, Y, logdet
end

# Inverse pass and compute gradients
function cond_mshint_inverse(Zx, Zy, AN_X, AN_Y, CL, XY_dims, L, K, split_scales)
    split_scales && ((XY_save, Zx, Zy) = split_states(XY_dims, Zx, Zy))
    for i=L:-1:1
        if split_scales && i < L
        ...
        for j=K:-1:1
            Zx_, Zy_ = CL[i, j].inverse(Zx, Zy)
            Zy = AN_Y[i, j].inverse(Zy_)
            Zx = AN_X[i, j].inverse(Zx_)
        end
...
function cond_mshint_backward(ΔZx, ΔZy, Zx, Zy, AN_X, AN_Y, CL, XY_dims, L, K, split_scales)

    # Split data and gradients
    if split_scales
        ΔXY_save, ΔZx, ΔZy = split_states(XY_dims, ΔZx, ΔZy)
        XY_save, Zx, Zy = split_states(XY_dims, Zx, Zy)
    end
    ...
    for i=L:-1:1
        ...
        for j=K:-1:1
            ΔZx_, ΔZy_, Zx_, Zy_ = CL[i, j].backward(ΔZx, ΔZy, Zx, Zy)
            ΔZx, Zx = AN_X[i, j].backward(ΔZx_, Zx_)
            ΔZy, Zy = AN_Y[i, j].backward(ΔZy_, Zy_)
        end
...
# Forward pass and compute logdet
function cond_mshint_forward_Y(Y, AN_Y, CL, XY_dims, L, K, split_scales)
    split_scales && (Y_save = Array{Array}(undef, L-1))
    for i=1:L
        Y = wavelet_squeeze(Y)
        for j=1:K
            Y_, logdet2 = AN_Y[i, j].forward(Y)
            Y = CL[i, j].forward_Y(Y_)
        end
        if split_scales && i < L    # don't split after last iteration
            Y, Zy = tensor_split(Y)
            Y_save[i] = Zy
    ...
    split_scales && (Y = cat_states(Y_save, Y))
    return Y

end

# Inverse pass and compute gradients
function cond_mshint_inverse_Y(Zy, AN_Y, CL, XY_dims, L, K, split_scales)
    split_scales && ((Y_save, Zy) = split_states(XY_dims, Zy))
    for i=L:-1:1
        if split_scales && i < L
            Zy = tensor_cat(Zy, Y_save[i])
        end
        for j=K:-1:1
            Zy_ = CL[i, j].inverse_Y(Zy)
            Zy = AN_Y[i, j].inverse(Zy_)
        end
        Zy = wavelet_unsqueeze(Zy)
    end
