...
export CondCouplingLayerGlow, CondCouplingLayerGlow3D
...
"""
struct CondCouplingLayerGlow <: NeuralNetLayer
    C::Conv1x1
...
@Flux.functor CondCouplingLayerGlow

# Constructor from 1x1 convolution and residual block
function CondCouplingLayerGlow(C::Conv1x1, RB::ResidualBlock; logdet=false, activation::ActivationFunction=SigmoidLayer())
...
CondCouplingLayerGlow(C::Conv1x1, RB::FluxBlock; logdet=false, activation::ActivationFunction=SigmoidLayer()) = CondCouplingLayerGlow(C, RB, logdet, activation)

# Constructor from input dimensions
function CondCouplingLayerGlow(n_in::Int64, n_c::Int64, n_hidden::Int64; k1=3, k2=1, p1=1, p2=0, s1=1, s2=1, logdet=false, activation::ActivationFunction=SigmoidLayer(), ndims=2)
...
CouplingLayerGlow3D(args...;kw...) = CondCouplingLayerGlow(args...; kw..., ndims=3)

# Forward pass: Input X, Output Y
function forward(X::AbstractArray{T, 4}, C::AbstractArray{T, 4}, L::CondCouplingLayerGlow) where T
...
function inverse(Y::AbstractArray{T, 4}, C::AbstractArray{T, 4}, L::CondCouplingLayerGlow; save=false) where T
...
function jacobian(ΔX::AbstractArray{T, 4}, Δθ::Array{Parameter, 1}, X, L::CondCouplingLayerGlow) where T
...
function adjointJacobian(ΔY::AbstractArray{T, N}, Y::AbstractArray{T, N}, L::CondCouplingLayerGlow) where {T, N}
...
# Clear gradients
function clear_grad!(L::CondCouplingLayerGlow)
    clear_grad!(L.C)
    clear_grad!(L.RB)
end

# Get parameters
function get_params(L::CondCouplingLayerGlow)
    p1 = get_params(L.C)
    p2 = get_params(L.RB)
