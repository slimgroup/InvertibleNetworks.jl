...
# Constructor
function ResidualBlock(n_in, n_out,  n_hidden; k1=3, k2=3, p1=1, p2=1, s1=1, s2=1, GALU=false, ndims=2)
    ...
    return ResidualBlock(W1, W2, W3, b1, b2, GALU, (s1, s2), (p1, p2))
end

# Constructor for given weights
function ResidualBlock(W1, W2, W3, b1, b2; p1=1, p2=1, s1=1, s2=1, GALU=false, ndims=2)
...
function forward(X1::AbstractArray{T, N}, RB::ResidualBlock; save=false) where {T, N}
    ...
    # Finish forward
    RB.GALU == true ? (return GaLU(Y3)) : (return Y3) 
end
...
function backward(ΔX4::AbstractArray{T, N}, X1::AbstractArray{T, N},
    ...
    #RB.GALU == true ?  (ΔY3 = GaLUgrad(ΔX4, Y3)) : (ΔY3 = LeakyReLUgrad(ΔX4, Y3))
    RB.GALU == true ?  (ΔY3 = GaLUgrad(ΔX4, Y3)) : (ΔY3 = ΔX4)
