...
function ResNet(n_in::Int64, n_hidden::Int64, nblocks::Int64; k::Int64=3, p::Int64=1, s::Int64=1, norm::Union{Nothing, String}="batch", n_out::Union{Nothing, Int64}=nothing)
    ...
    for i = 1:nblocks-1
        ...
        # Skip-connection
        resnet_blocks[i]   = SkipConnection(Chain(Conv((k, k), n_in => n_hidden; stride = s, pad = p), NormLayer, x->relu.(x), Conv((k, k), n_hidden => n_in; stride = s, pad = p)), +)
    ...
    if isnothing(n_out)
        ...
        # Skip-connection
        resnet_blocks[end] = SkipConnection(Chain(Conv((k, k), n_in => n_hidden; stride = s, pad = p), NormLayer, x->relu.(x),Conv((k, k), n_hidden => n_in; stride = s, pad = p)), +)
