...
function actnorm_forward(X, k, s, b, logdet)
    ...
    logdet == true ? (return Y, logdet_forward(nx, ny, batchsize, s)) : (return Y)
end
...
function actnorm_backward(ΔY, Y, k, s, b, logdet)
    ...
    logdet == true && (Δs -= logdet_backward(nx, ny, batchsize, s))
...
# Logdet
logdet_forward(nx, ny, batchsize, s) = nx*ny*batchsize*sum(log.(abs.(s.data)))
logdet_backward(nx, ny, batchsize, s) = nx*ny*batchsize./ s.data
