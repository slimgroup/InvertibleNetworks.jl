...
See also: [`∇mse`](@ref)
"""
mse(X::Array{Float32, N}, Y::Array{Float32, N}) where N = .5f0/size(X, N)*norm(X - Y, 2)^2
...
"""
log_likelihood(X::Array{Float32, N}; μ=0f0, σ=1f0) where N = 1f0/size(X, N)*sum(-.5f0*((X .- μ)/σ).^2)
...
See also: [`log_likelihood`](@ref)
"""
∇log_likelihood(X::Array{Float32, N}; μ=0f0, σ=1f0) where N = -1f0/size(X, N)*(X .- μ)/σ^2
...
See also: [`log_likelihood`](@ref)
"""
function Hlog_likelihood(X::Array{Float32, N}; μ=0f0, σ=1f0) where N
    return InvertibleNetworkLinearOperator{Array{Float32, N},Array{Float32, N}}(
